...
__state_dim__ = 3
__num_controls__ = 1
__policy_path__ = './time_dependent_mountain_car.ckpt'
...
def plot_controls():
    ...
    for i, y_val in enumerate(y_axis):
        for j, x_val in enumerate(x_axis):
            controls = control_func(transform_observation(np.array([x_val, y_val, 1.0])))
...
def plot_value():
    ...
    for i, y_val in enumerate(y_axis):
        for j, x_val in enumerate(x_axis):
            surface[i, j] = 100.0 * value_func(transform_observation(np.array([x_val, y_val, 1.0])))
...
def rand():
    ...
    rl.tools.render(game, control_func=control, path='./' + 'time_dependent_' + __gym_env_name__ + '_random.gif')
